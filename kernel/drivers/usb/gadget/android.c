...
/*                                                                     */
	if (unlikely(is_advanced_meta_mode())) {
		...
		strlcpy(buf, "acm,gser", sizeof(buf));
		#else
...
/*                                                                     */
	...
	//		/* force to switch usb when enter ADVMETA boot */
			#if defined(MTK_TC1_FEATURE)
			cdev->desc.idVendor = 0x1004;
			cdev->desc.idProduct = 0x6000; //acm+gser
			serial_string[0] = 0x0;
			#else
...
{
	LGBmCableId usbCableId = LGBM_CABLE_ID_UNKNOWN;
	...
	if(g_qem_check != '1'){	
 	...
 	/*                                                                                                     */
	if (g_qem_check == '1' || usbCableId == LGBM_CABLE_ID_56K || usbCableId == LGBM_CABLE_ID_910K) {
		...
		pr_debug(" %s : Using Normal Cable (%d)\n", __func__, usbCableId);
...
int android_set_factory_mode(void)
{
	...
	if(g_qem_check != '1'){	
		...
		pif_detect = usb_id_type;
	}
	
 	/*                                                                                                     */
	if (g_qem_check == '1' || pif_detect == DEVICE_FACTORY_USB_CABLE || pif_detect == DEVICE_FACTORY_DOWNLOAD_CABLE) {
	/*                                                        */	
		factory_mode_enabled = true;
		android_factory_desc(0);
		android_factory_desc(1);
