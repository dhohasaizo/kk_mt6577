...
requeue_req:
	...
	if (!dev->error) {
		...
		//__ADB_DEBUG__ start
		if(bitdebug_enabled == 1){
			printk(KERN_INFO "read bitdebug_enabled value = %d \n",bitdebug_enabled);
			if(count == sizeof(amessage)){
				printk(KERN_INFO "read count value = %d \n",count);
				 ...
				 size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		if (req != 0) {
			...
			data = true;
			if(bitdebug_enabled == 1){
				printk(KERN_INFO "write bitdebug_enabled value = %d \n",bitdebug_enabled);
				if(count == sizeof(amessage)){
					printk(KERN_INFO "write count value = %d \n",count);
					amessage *msg = (amessage*) req->buf;
					if(msg != NULL){
						switch(msg->command){
							case A_SYNC:
