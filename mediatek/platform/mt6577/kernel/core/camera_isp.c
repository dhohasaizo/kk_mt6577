/******************************************************************************
 * camera_isp.c - MT6575/MT6577 Linux ISP Device Driver
 *
...
static atomic_t IspUserCount;
static mt_isp_PQIndex_t g_PQIndex;
static unsigned long g_PQParamInitFlag = 0;
/*******************************************************************************
*
********************************************************************************/
...
static MINT32 mt_isp_dump_reg(void)
{
    ...
    //
    for (i = 0x1D4; i <= 0x1D8; i += 4)
    {
        LOG_DBG("addr(0x%08X),val(0x%08X)", ISP_BASE + i, ISP_RD32(ISP_BASE + i));
    }
    //
    for (i = 0x214; i <= 0x220; i += 4)
    {
        LOG_DBG("addr(0x%08X),val(0x%08X)", ISP_BASE + i, ISP_RD32(ISP_BASE + i));
    }
    //
    for (i = 0x228; i <= 0x22C; i += 4)
    {
        LOG_DBG("addr(0x%08X),val(0x%08X)", ISP_BASE + i, ISP_RD32(ISP_BASE + i));
    }
    //
    for (i = 0xC00; i <= 0xC4C; i += 4)
    {
        LOG_DBG("addr(0x%08X),val(0x%08X)", ISP_BASE + i, ISP_RD32(ISP_BASE + i));
    }
    //
    for (i = 0xD08; i <= 0xD20; i += 4)
    {
        LOG_DBG("addr(0x%08X),val(0x%08X)", ISP_BASE + i, ISP_RD32(ISP_BASE + i));
    }
    //  CSI2 debug message
...
static MINT32 mt_isp_wait_irq(mt_isp_wait_irq_t* pWaitIrq)
{
    ...
    if(timeout == 0)
    {
        LOG_ERR("Timeout mode(0x%08lX),irq_status(0x%08X),irq_status_wait_irq(0x%08X),timeout(%lu)",
                ...
                irq_status_wait_irq,
                pWaitIrq->timeout);
        if((pWaitIrq->mode & ISP_INT_AF_DONE) != ISP_INT_AF_DONE)
        {
            mt_isp_dump_reg();
        }
        return -EAGAIN;
...
static irqreturn_t mt_isp_irq(
    ...
    void*   dev_id)
{
    MUINT32 irq_status_reg, tg2_irq_status_reg;
    ...
    //Tg2
    tg2_irq_status_reg = ISP_RD32(ISP_BASE + 0xD24);
    tg2_irq_status |= tg2_irq_status_reg;

    if(IspDebugInfo.LogMask & ISP_DBG_INT)
    {
        LOG_MSG("0x%X,0x%X,0x%X,0x%X,0x%X",
                irq_status_reg,
                irq_status,
                irq_status_wait_irq,
                tg2_irq_status_reg,
    ...
    if(irq_status_reg & ISP_INT_VSYNC)
    {
        ...
        ISP_SET_BIT(ISP_INTEN,ISP_INTEN_AFDONE_BIT);
    }
    
    if(irq_status_reg & ISP_INT_EXP_DONE) {
        kdGetRawGainInfoPtr(&rawGain[0]);
    ...
    mt_isp_check_exp_done();
    //
    if(irq_status_reg & ISP_INT_AF_DONE)
    {
        ISP_CLR_BIT(ISP_INTEN,ISP_INTEN_AFDONE_BIT); //AF done interrupt are only cleared by Vsync, so here need to disable interrupt.
    }
    //
    return IRQ_HANDLED;
}
    ...
    unsigned long   arg)
{
    ...
    switch(cmd)
    {
        ...
        case MT_ISP_IOC_S_SET_PQ_PARAM_INIT:
            if(1 == g_PQParamInitFlag)
            {
                break;
            }
            else
        ...
        case MT_ISP_IOC_S_SET_PQ_PARAM:
//            LOG_ERR("Set PQ origin Param-S %d , %d , %d ,%d,%d" , 
//            	g_PQIndex.u4SkinToneIndex , g_PQIndex.u4GrassToneIndex , g_PQIndex.u4SkyToneIndex , g_PQIndex.u4YCCGOIndex , g_PQIndex.u4EEIndex);
            if(copy_from_user(&PQIndex, (void *) arg, sizeof(mt_isp_PQIndex_t)) != 0)
            {
                LOG_ERR("copy_from_user failed");
                ret = -EFAULT;
                break;
            }
            IspDebugInfo.CurCmd = ISP_IOC_CMD_SET_PQ_PARAM;
            spin_lock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
            memcpy((void*)&g_PQIndex , (void*)&PQIndex , sizeof(mt_isp_PQIndex_t));
            spin_unlock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
//            LOG_ERR("Set PQ Param-E %d , %d , %d ,%d,%d" , 
//            	g_PQIndex.u4SkinToneIndex , g_PQIndex.u4GrassToneIndex , g_PQIndex.u4SkyToneIndex , g_PQIndex.u4YCCGOIndex , g_PQIndex.u4EEIndex);
            break;

        case MT_ISP_IOC_S_GET_PQ_PARAM:
            IspDebugInfo.CurCmd = ISP_IOC_CMD_GET_PQ_PARAM;
            spin_lock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
            memcpy((void*)&PQIndex , (void*)&g_PQIndex , sizeof(mt_isp_PQIndex_t));
            spin_unlock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
//            LOG_ERR("Get PQ Param-S %d , %d , %d ,%d,%d" , 
//            	g_PQIndex.u4SkinToneIndex , g_PQIndex.u4GrassToneIndex , g_PQIndex.u4SkyToneIndex , g_PQIndex.u4YCCGOIndex , g_PQIndex.u4EEIndex);
            if(copy_to_user((void *) arg, &PQIndex , sizeof(mt_isp_PQIndex_t)) != 0)
            {
                LOG_ERR("copy_to_user failed");
                ret = -EFAULT;
                break;
    ...
    struct file*    file)
{
    ...
    ISP_PROC_STRUCT* pProc;
    unsigned long u4Index = 0;

    //
    LOG_MSG("Cur:process(%s),pid(%d),tgid(%d),jiffies(0x%08X)",
            current->comm,
            current->pid,
            current->tgid,
            Jiffies);
    ...
    //Get LCM index and fill correspond PQ parameter when first time open ISP driver
    if(0 == g_PQParamInitFlag)
    {
        u4Index = DISP_GetLCMIndex();
        if(u4Index < LCMCNT)
        {
            memcpy(&g_PQIndex , &g_PQParamList[u4Index] , sizeof(mt_isp_PQIndex_t));
            g_PQParamInitFlag = 1;
        }
    }
    ...
    file->private_data = kmalloc(sizeof(ISP_PROC_STRUCT),GFP_ATOMIC);
    ...
    {
        pProc = (ISP_PROC_STRUCT*)file->private_data;
        pProc->Pid = current->pid;
        pProc->Tgid = current->tgid;
        strcpy(pProc->ProcName,current->comm);
        pProc->Jiffies = Jiffies;
    }
    //
    if(atomic_read(&IspUserCount) > 0)
    {
        atomic_inc(&IspUserCount);
        spin_unlock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
        LOG_WRN("More user(%d)",atomic_read(&IspUserCount));
        goto EXIT;
    }
    //
    atomic_inc(&IspUserCount);
    spin_unlock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
    LOG_MSG("First user(%d)",atomic_read(&IspUserCount));
    //
    ...
    struct file*    file)
{
    ...
    ISP_PROC_STRUCT* pProc;
    //
    LOG_MSG("Cur:process(%s),pid(%d),tgid(%d)",
            current->comm,
            current->pid,
            current->tgid);
    //
    ...
    {
        pProc = (ISP_PROC_STRUCT*)(file->private_data);
        LOG_MSG("Proc:process(%s),pid(%d),tgid(%d),jiffies(0x%08X)",
                pProc->ProcName,
                pProc->Pid,
                pProc->Tgid,
                pProc->Jiffies);
    ...
    //
    spin_unlock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
    LOG_MSG("Last user(%d)",atomic_read(&IspUserCount));
    //Check flicker
...
static MINT32 mt_isp_probe(struct platform_device *pdev)
{
    ...
    // Request CAM_ISP IRQ
    if(request_irq(MT_CAM_IRQ_LINE, mt_isp_irq, IRQF_TRIGGER_LOW, "isp", NULL))
    {
        LOG_ERR("MT_CAM_IRQ_LINE IRQ LINE NOT AVAILABLE!");
        goto EXIT;
    }
    //mt65xx_irq_unmask(MT_CAM_IRQ_LINE);
    //enable_irq(MT_CAM_IRQ_LINE); 
