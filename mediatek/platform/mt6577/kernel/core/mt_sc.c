...
{
	char str[128] = { 0 };
	bool eint_wake = false, ccif_md_wake = false;
...
/*
 * wake_src: WAKE_SRC_XXX in sc.h for suspend
 * wake_irq: wakeup source's IRQ and 0 means to disable this wakeup source
 */
void sc_set_wake_src(u32 wake_src, u16 wake_irq)
{
	...
	sc_xinfo("wake_src = 0x%x, wake_irq = %u\n", wake_src, wake_irq);

#ifdef CONFIG_MTK_SC_DEBUG
	if (wake_src & WAKE_SRC_CFG_KEY)
#else
	if (!(wake_src & WAKE_SRC_CFG_KEY))
#endif
		return;

	for (i = 1; i < NUM_WAKE_SRC; i++) {
		if ((i >= 10 && i <= 18) || i == 20)
			continue;
		...
		if (wake_src & mask) {
			...
			if (mask != WAKE_SRC_CA9_DBG)	/* has IRQ */
