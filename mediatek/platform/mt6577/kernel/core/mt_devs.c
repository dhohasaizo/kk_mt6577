...
struct platform_device sensor_irrc = {
	.name	       = "irrc",
	.id            = -1,
};
...
static struct platform_device lp5521_led_dev = {
    .name = "lp5521",
    .id   = -1,
};
#endif
...
static struct platform_device miniabb_device = {
    .name   = "miniabb",
    .id     = -1,
};
static struct platform_device miniabb_i2c = {
    .name = "miniabb_i2c",
    .id   = -1,
};
#endif
...
#if defined(CONFIG_EXT_CHARGER_SUPPORT) /*                                 */
static struct platform_device ext_charger_device = {
    .name   = "ext_charger",
    .id     = -1,
};
static struct platform_device ext_charger_i2c = {
    .name = "ext_charger_i2c",
    .id   = -1,
};
#endif
...
static struct platform_device charging_ic_device = {
	.name	= "single_charger",
	.id		= -1,
};
#endif
...
/*                                                           */
#if defined(LGE_USE_DOME_KEY)
	retval = platform_device_register(&lp5521_led_dev);
	if (retval != 0){
		return retval;
	}
#endif
...
//                                                                  
#if defined(CONFIG_MAX8971_CHARGER)
retval = platform_device_register(&charging_ic_device);
	if(retval != 0) {
		printk("[MT6575_devs] Unable to device register (%d)\n", retval);
		return retval;
	}
	else
	{
		printk("[MT6575_devs] charging_ic_device OK\n");
	}
#endif
...
/*=======================================================================*/
#if defined(CONFIG_MINIABB_CHARGER) /*                                 */
    retval = platform_device_register(&miniabb_device);
    if(retval != 0){
        return retval;
    }
    retval = platform_device_register(&miniabb_i2c);
    if(retval != 0){
        return retval;
    }
    ...
    retval = platform_device_register(&sensor_irrc);
		printk("irrc device!");
	if (retval != 0)
		return retval;
