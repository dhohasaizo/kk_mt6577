...
#ifdef MTK_TC1_FEATURE
    if( host->force_check_card_state == true )
    {
        host->force_check_card_state = false; 
        if ( host->block_damage_card == true )
        {
            goto end;
        }
    }    
...
static int msdc_lower_freq(struct msdc_host *host)
{
    ...
    if (div >= MSDC_MAX_FREQ_DIV) {
        ...
        if(host->id == SD_HOST_ID){
			spin_lock_irqsave(&host->state_check_lock,flags);
			host->block_damage_card = true;// sd card damage
 			host->force_check_card_state = true;
			host->card_inserted = 0;
			card_present = host->card_cd_pin_state;
        	if(host->mmc->card)
                mmc_card_set_removed(host->mmc->card);        
			spin_unlock_irqrestore(&host->state_check_lock,flags);
...
static int msdc_ops_get_cd(struct mmc_host *mmc)
{
...
#ifdef MTK_TC1_FEATURE
    if ( true == host->block_damage_card )
    {
        ERR_MSG("msdc_ops_get_cd : block_damage_card = true"); 
        goto end;
    }
#endif /* MTK_TC1_FEATURE */
    ...
    if (host->hw->flags & MSDC_CD_PIN_EN) { /* for card, MSDC_CD_PIN_EN set*/
        ...
        if(!host->hw->cd_level)
            ...
            host->card_inserted = (sdr_read32(MSDC_PS) & MSDC_PS_CDSTS) ? 1 : 0; 
#else
        host->card_inserted = (sdr_read32(MSDC_PS) & MSDC_PS_CDSTS) ? 0 : 1; 
#endif /* MTK_TC1_FEATURE */
    } else {
...
#ifdef MTK_TC1_FEATURE
static bool msdc_get_cd_pin_state(struct msdc_host *host)
{
	u32 base = host->base;
	u32 status = 0;
	...
	status = sdr_read32(MSDC_PS);
	if(!host->hw->cd_level)
        return (status & MSDC_PS_CDSTS) ? 0 : 1;
    else
        return (status & MSDC_PS_CDSTS) ? 1 : 0;
}
#endif /* MTK_TC1_FEATURE */
...
#ifdef MTK_TC1_FEATURE
		spin_lock_irqsave(&host->state_check_lock,flags);
		host->card_cd_pin_state = msdc_get_cd_pin_state(host);
		if(host->card_cd_pin_state == 0){
			got_bad_card = host->block_damage_card;
			host->card_inserted = 0; 
        	if(host->mmc->card)
               mmc_card_set_removed(host->mmc->card);  
		}
...
#ifdef MTK_TC1_FEATURE
    spin_lock_init(&host->state_check_lock);    // avoid race condition between real hot-plug and fake hot-plug    
#endif /* MTK_TC1_FEATURE */
...
#ifdef MTK_TC1_FEATURE
	host->block_damage_card = false;        // card normal, no damage    
	host->force_check_card_state = false;    
#endif /* MTK_TC1_FEATURE */
