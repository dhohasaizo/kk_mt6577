...
/*                                                                              */
#if 1
            //if(DL1_Restart == KAL_TRUE)
            {
                HW_Read_idx = Afe_Get_Reg(AFE_DL1_CUR) - Afe_Get_Reg(AFE_DL1_BASE);
                xlog_printk(ANDROID_LOG_INFO, "Sound", "AudDrv_write, HW_Read_idx=0x%x", HW_Read_idx);
                // To avoid SW write and HW read the same pointer, which will cause sound noisy!
                if(HW_Read_idx >= (AFE_dl_Control->rBlock.u4BufferSize>>1))
                {
                    Afe_Block->u4WriteIdx = 0;
                }
                else
                {
                    Afe_Block->u4WriteIdx = AFE_dl_Control->rBlock.u4BufferSize>>1; 	 
                }
                //    DL1_Restart = KAL_FALSE;
            }
#endif
/*                                                                              */
			spin_lock_irqsave(&auddrv_DL1Ctl_lock, flags);
			...
			if(Afe_WriteIdx_tmp + copy_size < Afe_Block->u4BufferSize ) // copy once
         {
            ...
            {
			// add log for music speaker noise
                xlog_printk(ANDROID_LOG_INFO, "Sound", "AudDrv_write Afe_WriteIdx_tmp=0x%x, HW r_idx:0x%x \n", Afe_WriteIdx_tmp, Afe_Get_Reg(AFE_DL1_CUR) - Afe_Get_Reg(AFE_DL1_BASE));
         ...
         else  // copy twice
         {
			...
			// add log for music speaker noise
            xlog_printk(ANDROID_LOG_INFO, "Sound", "AudDrv_write2 Afe_WriteIdx_tmp=0x%x, HW r_idx:0x%x \n", Afe_WriteIdx_tmp, Afe_Get_Reg(AFE_DL1_CUR) - Afe_Get_Reg(AFE_DL1_BASE));
...
// MTK_SWIP_PROTECT_START
    len += sprintf(buf+ len, "AFE_PRED_0    = 0x%x\n",Afe_Get_Reg(0x0260));
    len += sprintf(buf+ len, "AFE_PRED_1    = 0x%x\n",Afe_Get_Reg(0x0264));
// MTK_SWIP_PROTECT_END
