...
static spinlock_t tbl_charger_otg_vbus_lock;
static int tbl_charger_otg_vbus_mode = 0;
static void ncp1851_set_otg_en_task(struct work_struct *data);
static DECLARE_WORK(ncp1851_set_otg_en_work, ncp1851_set_otg_en_task);
static void ncp1851_set_otg_en_task(struct work_struct *data)
{
    ncp1851_set_otg_en(0);
    ncp1851_set_otg_en(tbl_charger_otg_vbus_mode);
}
void tbl_charger_otg_vbus(kal_uint32 mode)
{
kal_uint8 usb_err, otg_mode;
unsigned long flags;

    usb_err = mode>>8;
    otg_mode = mode & 0xFF;
    spin_lock_irqsave(&tbl_charger_otg_vbus_lock, flags);
    tbl_charger_otg_vbus_mode = otg_mode;
    spin_unlock_irqrestore(&tbl_charger_otg_vbus_lock, flags);
    if(usb_err & 0x02)
		{
      printk("m:%08X:set\n", mode);
      ncp1851_set_otg_en(otg_mode);
		}
		else
		{
      printk("m:%08X:work\n", mode);
      schedule_work(&ncp1851_set_otg_en_work);
		}
};
#endif
...
int PMIC_IMM_GetOneChannelValue(int dwChannel, int deCount)
{
	...
	kal_int32 u4channel[5] = {0,0,0,0,0};
	...
	else if( dwChannel == AUXADC_HW_OCV_CHANNEL )
	{
	    count=0;
	    while(upmu_auxadc_get_wakeup_date_ready() != 1)
	    {
			if( (count++) > count_time_out)
			{
				xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[IMM_GetOneChannelValue_PMIC_4] Time out! HW OCV not ready!\r\n");
				break;
			}	    
	    }
		temp_data_7_0=upmu_auxadc_get_wakeup_date_7_0();
		ret_data = temp_data_7_0;
		temp_data_9_8=upmu_auxadc_get_wakeup_date_9_8();
		ret_data |= (temp_data_9_8 << 8);

		u4channel[4] = ret_data;
	}
	else
	{
...
void BAT_thread_ncp1851(void)
{
    ...
    if(g_battery_thermal_throttling_flag==1)
    {
        ...
        {
 #if defined(CONFIG_POWER_EXT)
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY:ncp1851] CONFIG_POWER_EXT, no update mt6329_battery_update_power_down.\n");
 #else
...
void PrechargeCheckStatus(void)
{
    ...
    struct mt6329_battery_data *bat_data;
    int chr_err_cnt = 0;
    ...
    while(1)
    {
        if(g_pmic_init_for_ncp1851 == 1) //make sure mt6329_battery_probe register finished
	     break;
        else
            msleep(1000);
    }
...
static int __init mt6329_battery_init(void)
{
    int ret;
    #ifdef CONFIG_USB_MTK_HDRC_HCD
    spin_lock_init(&tbl_charger_otg_vbus_lock);
    #endif 
...
MODULE_DESCRIPTION("MT6329 Battery Device Driver NCP1851");
MODULE_LICENSE("GPL");
