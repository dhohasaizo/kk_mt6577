...
kal_bool initChargingFull = KAL_FALSE;
//                                                                                    

//                                                                                                     
/*Sky Yang's new algorithm modification.*/
#define MTK_C_ADJ_NEW
//                                                                                                     
...
extern int bat_soc_tracker;
extern int battery_info_read(void);
#endif
//                                                                              
...
int fgauge_get_saddles(void)
{
    ...
    return sizeof(battery_profile_t2) / sizeof(BATTERY_PROFILE_STRUC);
        ...
        return sizeof(battery_profile_2nd_t2) / sizeof(BATTERY_PROFILE_STRUC);    
    }    
    //                                                                  
}
...
int fgauge_get_saddles_r_table(void)
{
    ...
    return sizeof(r_profile_t2) / sizeof(R_PROFILE_STRUC);
        ...
        return sizeof(r_profile_2nd_t2) / sizeof(BATTERY_PROFILE_STRUC);    
    }
    //                                                                  
}
...
//                                                                   
    ...
    {
        case TEMPERATURE_T0:
            return &battery_profile_2nd_t0[0];
            break;    
        case TEMPERATURE_T1:
            return &battery_profile_2nd_t1[0];
            break;
        case TEMPERATURE_T2:
            return &battery_profile_2nd_t2[0];
            break;
        case TEMPERATURE_T3:
            return &battery_profile_2nd_t3[0];
            break;
        case TEMPERATURE_T:
            return &battery_profile_2nd_temperature[0];
            break;
        default:
            return NULL;
            break;
...
//                                                                   
    ...
    {
        case TEMPERATURE_T0:
            return &r_profile_2nd_t0[0];
            break;
        case TEMPERATURE_T1:
            return &r_profile_2nd_t1[0];
            break;
        case TEMPERATURE_T2:
            return &r_profile_2nd_t2[0];
            break;
        case TEMPERATURE_T3:
            return &r_profile_2nd_t3[0];
            break;
        case TEMPERATURE_T:
            return &r_profile_2nd_temperature[0];
            break;
        default:
            return NULL;
            break;
...
/* R-Table (First Time) */    
    ...
    for (i = 0; i < saddles - 1; i++)
    {
        ...
        if ((voltage <= (profile_p+i)->voltage) && (voltage >= (profile_p+i+1)->voltage))
        {
            ...
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "DOD=%d, voltage=%d\r\n", dod0, voltage);
...
//                                                                                                     
    ...
    /*Calculate C factor for adjustment and it is percentage unit.*/
    if(C_0mA >= C_400mA)
    {
        C_factor = ((C_0mA - C_400mA) * 100)/C_400mA;
    }

    /*New algorithm will adjust Qmax value.*/
    FG_dod_1 =  gFG_DOD0 - ((gFG_columb*100)/(gFG_BATT_CAPACITY_aging - (C_factor * gFG_BATT_CAPACITY_aging/100)));
    xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "FG_dod1=%d, gFG_columb=%d\r\n", FG_dod_1, gFG_columb);
    xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "C_factor=%d, Mother=%d, Qmax_aging=%d.\r\n", C_factor, (gFG_BATT_CAPACITY_aging - (C_factor * gFG_BATT_CAPACITY_aging / 100)),gFG_BATT_CAPACITY_aging);
#else
...
//                                                                                 
#ifdef CONFIG_LGE_NVRAM_ALGORITHM
        /*Customer loading point for power on.*/
        bat_soc_tracker = battery_info_read(); //                                                     
        if( bat_soc_tracker > 0 && bat_soc_tracker <= 15 )  // read success and vaild data
        {
        
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "Power on loading check point.\r\n");
            xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "Loading check point bat_soc_tracker = %d, gFG_capacity_by_c = %d\r\n",
                    bat_soc_tracker, gFG_capacity_by_c);

            if( bat_soc_tracker < 15 )
            {
                if( bat_soc_tracker > gFG_capacity_by_c )
                {
                    if( ( bat_soc_tracker - gFG_capacity_by_c ) > 5 && ( bat_soc_tracker - gFG_capacity_by_c ) < 10 )
                    {
                        gFG_capacity = bat_soc_tracker;
                    }
                    else
                    {
                ...
                {
                    if( ( gFG_capacity_by_c - bat_soc_tracker ) > 5 && ( gFG_capacity_by_c - bat_soc_tracker ) < 10 )
                    {
                        gFG_capacity = bat_soc_tracker;
                    }
                    else
        ...
        //xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[Sky]Loading check point gFG_capacity = %d. \r\n", gFG_capacity);              
#else     
//                                                                                                     
#ifdef MTK_C_ADJ_NEW
        gFG_capacity = gFG_capacity_by_c;
...
//                                                                                    
    ...
    if (offChargingBatteryFull == '1') {
        initChargingFull = KAL_TRUE;
    }
    else {
        initChargingFull = KAL_FALSE;
    }
    
    if (Enable_FGADC_LOG == 1){
        xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] Battery Init Charging Full State : %d, offChargingBatteryFull: %c\n", initChargingFull, offChargingBatteryFull);
    }

    if(initChargingFull == KAL_TRUE) {
...
//                                                                   
    kal_int32 q_max_pos_50, q_max_pos_25, q_max_pos_0, q_max_neg_10;

    if(batt_id_check) /* LG CELL */
    {
        q_max_pos_50 = Q_MAX_POS_50;
        q_max_pos_25 = Q_MAX_POS_25;
        q_max_pos_0 = Q_MAX_POS_0;
        q_max_neg_10 = Q_MAX_NEG_10;
    }
    else  /* 2nd CELL */
    {
        q_max_pos_50 = Q_MAX_POS_50_2nd;
        q_max_pos_25 = Q_MAX_POS_25_2nd;
        q_max_pos_0 = Q_MAX_POS_0_2nd;
        q_max_neg_10 = Q_MAX_NEG_10_2nd;
    }
...
//                                                                   
    kal_int32 q_max_pos_50_h_current, q_max_pos_25_h_current, q_max_pos_0_h_current, q_max_neg_10_h_current;    

    if(batt_id_check) /* LG CELL */
    {
        q_max_pos_50_h_current = Q_MAX_POS_50_H_CURRENT;
        q_max_pos_25_h_current = Q_MAX_POS_25_H_CURRENT;
        q_max_pos_0_h_current = Q_MAX_POS_0_H_CURRENT;
        q_max_neg_10_h_current = Q_MAX_NEG_10_H_CURRENT;
    }
    else /* 2nd CELL */
    {
        q_max_pos_50_h_current = Q_MAX_POS_50_H_CURRENT_2nd;
        q_max_pos_25_h_current = Q_MAX_POS_25_H_CURRENT_2nd;
        q_max_pos_0_h_current = Q_MAX_POS_0_H_CURRENT_2nd;
        q_max_neg_10_h_current = Q_MAX_NEG_10_H_CURRENT_2nd;
    }
...
#else
//                                               
static ssize_t store_FG_Reset(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    if(buf != NULL && size != 0)
    {
        if(buf[0] == '1')
        {
            printk("[FuelGauge] RESET");
            fgauge_initialization();
            FGADC_thread_kthread();
        }
        else
        {
            printk("[FuelGauge] Not RESET");
        }
    }
    else
    {
...
static DEVICE_ATTR(FG_Reset, 0664, NULL, store_FG_Reset);
//                                               
#endif //                                              
...
//                                                                   
    if (batt_id_check)
    {
        CAR_TUNE_VALUE = 106; /* LG CELL */
    }
    else
    {
        CAR_TUNE_VALUE = 106; /* 2nd CELL */
    }
//                                                                   
#endif //                                              
...
#else
//                                               
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_FG_Reset);
//                                               
#endif //                                              
