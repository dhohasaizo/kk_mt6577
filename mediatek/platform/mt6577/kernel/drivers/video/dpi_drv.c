...
atomic_t dpi_vsync = ATOMIC_INIT(0);
atomic_t wait_dpi_vsync = ATOMIC_INIT(0);
...
{
#ifndef BUILD_UBOOT
	atomic_set(&wait_dpi_vsync, 1);
//	hrtimer_start(&hrtimer_vsync_dpi, ktime_set(0, VSYNC_US_TO_NS(vsync_timer_dpi)), HRTIMER_MODE_REL);
	wait_event_interruptible(_vsync_wait_queue_dpi, atomic_read(&dpi_vsync));
	atomic_set(&dpi_vsync, 0);
#endif
}
...
DPI_STATUS DPI_EnableInterrupt(DISP_INTERRUPT_EVENTS eventID)
{
    ...
    switch(eventID)
    {
        case DISP_DPI_FIFO_EMPTY_INT:
            DPI_REG->INT_ENABLE.FIFO_EMPTY = 0;
            break;
