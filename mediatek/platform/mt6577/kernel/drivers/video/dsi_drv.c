...
//                                                                                            
#ifdef HX8389B_QHD_DSI_VDO
void DSI_wait_2frame_end_L90(void)
{
  long int dsi_current_time;
  ...
  while(1)
  {
   ...
   if(get_current_time_us() - dsi_current_time > 40000)
   {
   	 printk("[DISP] %s : 40ms delay", __func__);
     break;
   }
   if(DSI_REG->DSI_STATE_DBG3.TCON_STATE == DSI_VDO_VFP_STATE)
   {
  	printk("[DISP] %s : DSI TCON_STATE is DSI_VDO_VFP_STATE", __func__);
   	break;
   }
  }
}
...
void DSI_set_cmdq_V3(LCM_setting_table_V3 *para_tbl, unsigned int size, unsigned char force_update)
{
	...
	DISP_LOG_PRINT(ANDROID_LOG_INFO, "DSI","DSI_set_cmdq_V3 ++ \n");
	do {
...
//                                                                          
#endif
			LCD_CHECK_RET(LCD_LayerSetByteSwap(FB_LAYER, false));
