...
int check_headset_type(void);  //check func
//                         
...
//                                                                                             
	ACCDET_DEBUG("[Accdet]eint work callback!!!!!!\n");
  ...
  //unmask EINT
	...
	gpio_state = mt_get_gpio_in(GPIO_ACCDET_EINT_PIN); /* 0 = low ( plugged-in ) */	 //GPIO_ACCDET_EINT_PIN  GPIO78
	ACCDET_DEBUG("[Accdet]GPIO_ACCDET_EINT = %d\n", gpio_state);	
	...
	if ( jack_detect_line_status != gpio_state )
	{
		...
		{
			ACCDET_DEBUG("[Accdet]Jack detect line changed to low\n");
			...
			if ( type == HEADSET_MIC )
			{
				ACCDET_DEBUG("[Accdet]JACK_TYPE = HEADSET_MIC\n");
			}
			else
			{
				ACCDET_DEBUG("[Accdet]JACK_TYPE = HEADSET_NO_MIC\n");
			}
		...
		{
			ACCDET_DEBUG("[Accdet]Jack detect line changed to high\n");
//                                                                     
			disable_accdet(); /* disable hook detection */
//                                                                     
			if ( ( jack_type == HEADSET_MIC ) && ( hook_key_pressed == 1 ) )
			{	
				ACCDET_DEBUG("[Accdet]HOOK_SWITCH = KEY_UP ( forced release caused by jack plugged-out\n");
			...
			ACCDET_DEBUG("JACK_TYPE = NO_DEVICE\n");
		...
		mt_eint_set_polarity(CUST_EINT_ACCDET_NUM, !jack_detect_line_status);	//CUST_EINT_ACCDET_NUM 5
		}
		else
		{
			ACCDET_DEBUG("Useless interrupt caused by noise, so skip\n");
		}
		mt_eint_unmask(CUST_EINT_ACCDET_NUM);
...
/*                                                                      */
    mt_eint_set_sens(CUST_EINT_ACCDET_NUM, CUST_EINT_LEVEL_SENSITIVE);
/*                                                                      */
    mt_eint_set_polarity(CUST_EINT_ACCDET_NUM, CUST_EINT_POLARITY_LOW);

//                                                                                                 
//	mt_eint_set_hw_debounce(CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_DEBOUNCE_CN);
    mt_eint_set_hw_debounce(CUST_EINT_ACCDET_NUM, 1); /*                                                                      */
    
	mt_eint_registration(CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_TYPE, accdet_eint_func, 0);

//	mt65xx_eint_registration(CUST_EINT_ACCDET_NUM, CUST_EINT_ACCDET_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, accdet_eint_func, 0);     //check
//                         
...
int check_headset_type(void)
{
	...
	ACCDET_DEBUG("[Accdet]check_headset_type\n");
	
	while( is_ready_to_detect_jack_type == 0 && loop_count < 30 ) /* need to set maximun wait time is under 5 sec */	
	{
		ACCDET_DEBUG("ACCDET_IRQ_STS = 0x%x, SI = %d, CI = %d, AB = %d, Loop Count = %d\n", INREG32(ACCDET_IRQ_STS), INREG32(ACCDET_SAMPLE_IN), 
																							INREG32(ACCDET_CURR_IN), INREG32(ACCDET_MEMORIZED_IN) & 0x3, loop_count);
	...
	{
		ACCDET_DEBUG("ACCDET is ready so read status to check jack type\n");
		...
		if ( regVal == 0 )
		{
			...
			ACCDET_DEBUG("[Accdet]HEADSET_NO_MIC was detected\n");
		}
		else if ( regVal == 1 )
		{
			type = HEADSET_MIC;

			ACCDET_DEBUG("[Accdet]HEADSET_MIC was detected\n");
		}
		else
		{
			type = HEADSET_MIC;

			ACCDET_DEBUG("[Accdet]Invalid register value of AB = %d, but assume that HEADSET_MIC was detected\n", regVal);
		}
	}
	else
	{
		...
		ACCDET_DEBUG("[Accdet]ACCDET is not ready than failed to detect headset type, but assume that HEADSET_MIC was detected\n", regVal);
	}
...
void check_hook_key(void)
{
	...
	ACCDET_DEBUG("[Accet]check_hook_key\n");

	current_status = INREG32(ACCDET_MEMORIZED_IN) & 0x3; //A=bit1; B=bit0    
	ACCDET_DEBUG("[Accdet]ACCDET_MEMORIZED_IN : AB = %d\n", current_status);
	...
	else if ( jack_type == HEADSET_MIC )
	{
		if ( current_status == 0 )
		{
			if ( hook_key_pressed == 0 )
			{
				...
				ACCDET_DEBUG("[Accdet]HOOK_SWITCH = KEY_DOWN\n");

				wake_lock_timeout(&accdet_key_lock, 2*HZ);    //set the wake lock
			}
			else
			{
				ACCDET_DEBUG("[Accdet]Unknown state ( Duplicated KEY_DOWN )\n");
			}
		}
		else if ( current_status == 1 )
		{
			if ( hook_key_pressed == 1 )
			{
				...
				ACCDET_DEBUG("[Accdet]HOOK_SWITCH = KEY_UP\n");
			}
			else
			{
				ACCDET_DEBUG("[Accdet]Unknown state ( Duplicated KEY_UP )\n");
			}
		}
		else
		{
			...
			{
				ACCDET_DEBUG("[Accdet]Unknown state ( AB = %d )\n", current_status);
			}
		}
	}
//                                                                              
	else if ( jack_type == HEADSET_NO_MIC)
	{
		if ( current_status == 1 )
		{
			...
			ACCDET_DEBUG("[Accdet]assume that jack_type is HEADSET_MIC\n");
		}
	}
//                                                                              
	else
	{
		ACCDET_DEBUG("[Accdet]Unknown state ( jack_type = %d, AB = %d )\n", jack_type, current_status);
	}	/* clear accdet interrupt */    

	while(INREG32(ACCDET_IRQ_STS) & IRQ_STATUS_BIT)	
	{
		ACCDET_DEBUG("[Accdet]ACCDET_IRQ_STS = 0x%x (   clearing )\n", INREG32(ACCDET_IRQ_STS));
...
//                                                                                     
	ACCDET_DEBUG( "[Accdet]accdet_work_callback\n");
