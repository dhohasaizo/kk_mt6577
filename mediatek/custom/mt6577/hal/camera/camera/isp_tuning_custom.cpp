...
//                   THH2  TH2  THL2                   THH1  TH1  THL1
    ...
    MINT32 THL1 = 3257;
    MINT32 THH1 = 3484;
    MINT32 TH1 = (THL1+THH1)/2; //(THL1 +THH1)/2
    MINT32 THL2 = 4673;
    MINT32 THH2 = 5155;
    MINT32 TH2 = (THL2+THH2)/2;//(THL2 +THH2)/2
    MINT32  DF_Thres = -70;
    MINT32 LV_Thres = 100;
    ...
    if(getSensorRole() == 0){ // For Main
        THL1 = 3257;
        THH1 = 3484;
        TH1 = (THL1+THH1)/2; //(THL1 +THH1)/2
        THL2 = 4800;//5000;
        THH2 = 5000;//5155;
        TH2 = (THL2+THH2)/2;//(THL2 +THH2)/2
        DF_Thres = -20;
        LV_Thres = 75;

    }else if (getSensorRole() == 1){ // For Sub
    	  THL1 = 3257;
        THH1 = 3484;
        TH1 = (THL1+THH1)/2; //(THL1 +THH1)/2
        THL2 = 4673;
        ...
        TH2 = (THL2+THH2)/2;//(THL2 +THH2)/2
         DF_Thres = -60;
        LV_Thres = 100;
    } else if ((getSensorRole() == 2)){ // For Backup sensor
    	  THL1 = 3257;
        THH1 = 3484;
        TH1 = (THL1+THH1)/2; //(THL1 +THH1)/2
        THL2 = 4800;
...
//#if ENABLE_MY_LOG
    //if  ( eIdx_Shading_CCT_old != eIdx_Shading_CCT_new )
    {
        ...
        XLOGD(
            "[-evaluate_Shading_CCT_index] DF_idx = %d, LV = %d\n"
            , i4DaylightFluorescentIndex, i4SceneLV
        );
    }
//#endif
...
if(eIdx_Shading_CCT_new ==  eIDX_Shading_CCT_D65){
	  if (eIdx_Shading_CCT_old == eIDX_Shading_CCT_CWF){
	  	  DF_Thres = DF_Thres + 50;
	  }
    if (( i4DaylightFluorescentIndex< DF_Thres) || (i4SceneLV < LV_Thres)){
        eIdx_Shading_CCT_new = eIDX_Shading_CCT_CWF;
        XLOGD(
            "[Shading CCT] Change table to CWF, DF_idx=%d\n"
            , i4DaylightFluorescentIndex
        );
    }
}

//Force to use TL84 table on A
if(eIdx_Shading_CCT_new ==  eIDX_Shading_CCT_ALight){
    eIdx_Shading_CCT_new = eIDX_Shading_CCT_CWF;
        XLOGD(
            "[Shading CCT] A Change table to CWF,i4CCT=%d\n"
            , i4CCT
        );
}
