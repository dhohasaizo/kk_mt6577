...
getParam_CRZ_Preview()
{
    static TuningParam_CRZ_T inst = {
        uUpScaleCoeff   : 8,    //  [5 bits; 1~19] Up sample coeff. choose > 12 may get undesirable result, '8' is recommended.
        uDnScaleCoeff   : 8,   // 15--->8 Down sample coeff. '1':blur,'19':sharpest; '15' is recommended.
    };
    char value[PROPERTY_VALUE_MAX] = {'\0'};
...
//	}
	...
	if(rOutput.u4ExpTimeInUS[0]<rInput.u4MinAEExpTimeInUS)
	{
		temp = static_cast<double>(rOutput.u4ExpTimeInUS[0])*1000.0/static_cast<double>(rInput.u4ShutterLineTime);
        temp = static_cast<MUINT32>(temp+0.5);
		rOutput.u4ExpTimeInUS[0] = static_cast<MUINT32>(rInput.u4ShutterLineTime*temp/1000) + 1;
...
#define LOG_TAG "HdrCustome"
	...
	XLOGD("Max Exposure Time 0EV = %d\nMax Sensor Gain 0EV = %d\n", rInput.u4MaxAEExpTimeInUS, rInput.u4MaxAESensorGain);
	XLOGD("Min Exposure Time 0EV = %d\nMin Sensor Gain 0EV = %d\n", rInput.u4MinAEExpTimeInUS, rInput.u4MinAESensorGain);
	XLOGD("Shutter Line Time = %d\n", rInput.u4ShutterLineTime);
...
#if (ENABLE_HDR_AE_DEBUG_INFO)	// Save HDR AE debug info to a file.
{
    ...
    pucLogBufPosition += strlen(pucLogBufPosition);
    ::sprintf(pucLogBufPosition, "Max Exposure Time Sensor= %d\nMax Sensor Gain Sensor= %d\n", rInput.u4MaxAEExpTimeInUS, rInput.u4MaxAESensorGain);
    pucLogBufPosition += strlen(pucLogBufPosition);
    ::sprintf(pucLogBufPosition, "Min Exposure Time Sensor= %d\nMin Sensor Gain Sensor= %d\n", rInput.u4MinAEExpTimeInUS, rInput.u4MinAESensorGain);
    pucLogBufPosition += strlen(pucLogBufPosition);
    ::sprintf(pucLogBufPosition, "u4ShutterLineTime = %d\n", rInput.u4ShutterLineTime);
    pucLogBufPosition += strlen(pucLogBufPosition);
