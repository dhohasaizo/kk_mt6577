...
#define SMALL_OBJECT_DETECTION_TUNNING_REG	(ts->finger_dsc.control_base+45) //0x0083
...
static unsigned int synaptics_rmi4_i2c_debug_mask = SYNAPTICS_RMI4_I2C_DEBUG_FINGER_STATUS |SYNAPTICS_RMI4_I2C_DEBUG_BUTTON_STATUS;
...
int synaptics_ts_read_f54(struct i2c_client *client, u8 reg, int num, u8 *buf)
{
	...
	if (synaptics_rmi4_i2c_debug_mask & SYNAPTICS_RMI4_I2C_DEBUG_NONE)
...
#else
			mt_eint_unmask(ts->pdata->irq_num);
#endif
		else
...
static int synaptics_ts_need_fw_upgrade(void)
{
	...
	SYNAPTICS_INFO_MSG("FW : cur_ver [%d], fw_ver [%d]\n", cur_ver, fw_ver);
...
//	ts->input_dev->name = "synaptics_ts";
	...
	if (likely(ts->pdata->use_irq && ts->pdata->irq_num)) {
		...
		mt65xx_eint_unmask(ts->pdata->irq_num);
#else
		mt_eint_set_sens(ts->pdata->irq_num, CUST_EINT_TOUCH_PANEL_SENSITIVE);
		mt_eint_set_hw_debounce(ts->pdata->irq_num, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);
		//mt_eint_registration(ts->pdata->irq_num, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, ts->pdata->irqflags, synaptics_eint_interrupt_handler, 1);
		mt_eint_registration(ts->pdata->irq_num, ts->pdata->irqflags, synaptics_eint_interrupt_handler, 1);
		mt_eint_unmask(ts->pdata->irq_num);
#endif
...
static struct tpd_driver_t tpd_device_driver = {
	.tpd_device_name = "synaptics_7020",
...
static struct synaptics_ts_platform_data l10_ts_data = {
	.use_irq = 1,
...
#else
	.num_of_button = 4,
	.button = {KEY_BACK,KEY_HOMEPAGE,KEY_MENU,KEY_SSK},
#endif
...
#else
 	i2c_register_board_info(0, &i2c_synaptics[0], 1);
#endif
