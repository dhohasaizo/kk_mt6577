...
//	int ret;
	...
	CPD_FUN();
	if(i2c_add_driver(&lm3639_driver))
	{
	...
	mt_set_gpio_dir(GPIO_BL_EN, GPIO_DIR_OUT); /* GPIO direction */


	mt_set_gpio_out(GPIO_BL_EN,GPIO_OUT_ONE);
	err = mt_get_gpio_out(GPIO_BL_EN);
	CPD_LOG("[ksw] %s, GPIO_BL_EN = %d\n", __func__, err);
	mdelay(10);
	...
	lm3639_smbus_read_byte(new_client, 0x0A, &data1);
	CPD_LOG("[ksw] %s, 0x%x i2c read = 0x%x\n", __func__, 0x0A, data1);

	data1 = 0x3F; //backlight config : 24v, linear, active low, 29.5mA
	err = lm3639_smbus_write_byte(new_client, 0x02, &data1);
	lm3639_smbus_read_byte(new_client, 0x02, &data1);
	CPD_LOG("[ksw] %s, 0x%x i2c read = 0x%x\n", __func__, 0x02, data1);

	data1 = 0x00; //IO control reg : all disable
	err = lm3639_smbus_write_byte(new_client, 0x09, &data1);
...
static int lm3639_pd_probe(struct platform_device *pdev)
{
    CPD_FUN();
	mt_set_gpio_mode(GPIO_BL_EN, GPIO_BL_EN_M_GPIO); /* GPIO mode */
	mt_set_gpio_pull_enable(GPIO_BL_EN, GPIO_PULL_ENABLE);
	mt_set_gpio_dir(GPIO_BL_EN, GPIO_DIR_OUT); /* GPIO direction */
...
//                                                  
    if(0/*BMT_status.temperature <= (-10)*/) //                                                                     
    {
