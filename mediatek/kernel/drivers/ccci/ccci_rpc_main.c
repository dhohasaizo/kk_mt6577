...
#undef CCCI_RPC_MSG
#define CCCI_RPC_MSG(fmt, args...)	printk("[ccci/rpc]"fmt, ##args)


#define  USE_RPC_SEQUENCIAL_RESPONSE
#ifdef USE_RPC_SEQUENCIAL_RESPONSE
DECLARE_WAIT_QUEUE_HEAD(rpc_response_waitq);
static atomic_t  rpc_response_done= ATOMIC_INIT(0);
#endif
...
static struct wake_lock	rpc_wake_lock;
...
_Get_next_fifo:      
		...
		if(!kfifo_out(&rpc_fifo, &buf_idx, sizeof(unsigned int))){
		   spin_unlock_bh(&rpc_fifo_lock);               
		   CCCI_RPC_MSG("--ccci_rpc_work--\n");
		  return;
		}
		 spin_unlock_bh(&rpc_fifo_lock);
		 CCCI_RPC_MSG("ccci_rpc_work loop=%d buf_idx=%d\n", dbg_loop_cnt++, buf_idx);
...
#ifdef USE_RPC_SEQUENCIAL_RESPONSE
		  wait_event_interruptible(rpc_response_waitq, atomic_read(&rpc_response_done)==1) ;
                  atomic_set(&rpc_response_done, 0);
	          CCCI_RPC_MSG("wake_up_rpc_work\n");
#endif
			goto _Get_next_fifo;
#endif         
		default:
			ccci_rpc_work_helper(&pkt_num, pkt, rpc_buf_tmp, tmp_data);
			CCCI_RPC_MSG("default_opid=%d\n", rpc_buf_tmp->op_id);
			break;
		}
_Next:
...
_send_return:   
#ifdef USE_RPC_SEQUENCIAL_RESPONSE
         atomic_set(&rpc_response_done, 1);
         wake_up_interruptible(&rpc_response_waitq);
         CCCI_RPC_MSG("send wake_up_rpc_work: %d\n",atomic_read(&rpc_response_done));
#endif	
	return ret;
}
...
{ 
    ...
    wake_lock_init(&rpc_wake_lock, WAKE_LOCK_SUSPEND, "ccci_rpc_main"); 
    return 0;
}
...
void __exit ccci_rpc_exit(void)
{
    ...
    wake_lock_destroy(&rpc_wake_lock);
